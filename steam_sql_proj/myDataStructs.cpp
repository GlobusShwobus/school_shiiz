...
namespace ORDO {
	...
	SQLSchema::SQLInsertOp SteamSummary::insertQuery() const
	{
		static constexpr std::string_view table =
	...
	SQLSchema::SQLCommand SteamGame::createTableCommand() const
	{
		...
		return SQLSchema::SQLCommand(table);
	}
	SQLSchema::SQLInsertOp SteamGame::insertQuery() const
	{
		static constexpr std::string_view table =
	...
	SQLSchema::SQLInsertOp SteamActivity::insertQuery() const
	{
		static constexpr std::string_view table =
			"INSERT INTO activity (game_id, name, total_playtime, playtime_fortnite) VALUES (?,?,?,?);";
		return SQLSchema::SQLInsertOp(table.data(), [this](sql::PreparedStatement* stmt) {
			stmt->setUInt64(1, gameID);
			stmt->setString(2, gameName);
	...
	SQLSchema::SQLInsertOp SteamAchievements::insertQuery() const
	{
		static constexpr std::string_view table =
